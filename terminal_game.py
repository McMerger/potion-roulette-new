...
self.player_life_lost_this_round = 0
self.ai_life_lost_this_round = 0
self.result_text = ""
...
elif self.player_hp <= 0 and self.ai_hp <= 0:
print("Result: Draw")
...
if self._handle_simultaneous_death(active_hand, brewer):
self._check_game_over()
else:
gained_cards = self._grant_random_cards(active_hand, 2)
print(f"{brewer} regains 2 random cards: {self._format_card_list(gained_cards)}")
self._check_game_over()
...
potion_a = self._craft_player_potion(self.player_hand, "Potion A")
if potion_a is None:
self.player_hand = snapshot
return None

potion_b = self._craft_player_potion(self.player_hand, "Potion B")
if potion_b is None:
self.player_hand = snapshot
return None
...
def _craft_player_potion(self, hand: list[str], potion_label: str) -> list[str] | None:
if len(hand) < 2:
print(f"{potion_label} defaults to Chaos + Chaos because you do not have enough cards to build it.")
return self._default_potion()

return self._prompt_potion_cards(hand, potion_label)
...
if len(self.ai_hand) < 2:
print("AI cannot build either potion, so both default to Chaos + Chaos.")
return self._default_potion(), self._default_potion()

if len(self.ai_hand) < 4:
print("AI cannot build both potions, so one defaults to Chaos + Chaos.")
real_potion = self._craft_ai_single_potion()
default_potion = self._default_potion()
if self.rng.randrange(2) == 0:
return real_potion, default_potion
return default_potion, real_potion
...
def _craft_ai_single_potion(self) -> list[str]:
scored_pairs: list[tuple[float, tuple[int, int]]] = []
default_potion = self._default_potion()
for used_indices in combinations(range(len(self.ai_hand)), 2):
potion = [self.ai_hand[used_indices[0]], self.ai_hand[used_indices[1]]]
score = self._score_ai_hidden_pair(potion, default_potion)
scored_pairs.append((score, used_indices))

best_score = max(score for score, _pair in scored_pairs)
best_pairs = [pair for score, pair in scored_pairs if score == best_score]
chosen_pair = self.rng.choice(best_pairs)
potion = [self.ai_hand[chosen_pair[0]], self.ai_hand[chosen_pair[1]]]
for index in sorted(chosen_pair, reverse=True):
self.ai_hand.pop(index)
return potion
...
return self._score_ai_hidden_pair(plan.potion_a, plan.potion_b)

def _score_ai_hidden_pair(self, potion_a: Sequence[str], potion_b: Sequence[str]) -> float:
potion_a_score = self._score_result_for_ai(self._evaluate_potion(*potion_a), target_is_player=True)
potion_b_score = self._score_result_for_ai(self._evaluate_potion(*potion_b), target_is_player=False)
choice_a = potion_a_score + potion_b_score

potion_b_player_score = self._score_result_for_ai(self._evaluate_potion(*potion_b), target_is_player=True)
potion_a_self_score = self._score_result_for_ai(self._evaluate_potion(*potion_a), target_is_player=False)
...
if result.kind == "damage":
lost_amount = self._deal_damage(target_is_player, result.amount)
print(f" {target_label} takes {lost_amount} damage from {result.label}.")
return

if result.kind == "heal":
healed_amount = self._heal_target(target_is_player, result.amount)
print(f" {target_label} heals {healed_amount} from {result.label}.")
return

if result.kind == "random_heal":
heal_player = self.rng.randrange(2) == 0
healed_amount = self._heal_target(heal_player, result.amount)
if heal_player:
print(f" Random heal from {result.label} goes to Player for {healed_amount}.")
else:
print(f" Random heal from {result.label} goes to AI for {healed_amount}.")
return

if result.kind == "random_damage":
damage_player = self.rng.randrange(2) == 0
lost_amount = self._deal_damage(damage_player, result.amount)
if damage_player:
print(f" Random damage from {result.label} hits Player for {lost_amount}.")
else:
print(f" Random damage from {result.label} hits AI for {lost_amount}.")
return
...
if random_effect == 0:
lost_amount = self._deal_damage(random_target_is_player, 1)
print(f" Chaos + Chaos damages {target_label} by {lost_amount}.")
return

if random_effect == 1:
healed_amount = self._heal_target(random_target_is_player, 1)
print(f" Chaos + Chaos heals {target_label} by {healed_amount}.")
return

if random_effect == 2:
lost_amount = self._deal_damage(random_target_is_player, 2)
print(f" Chaos + Chaos slams {target_label} for {lost_amount} damage.")
return
...
def _format_card_list(self, cards: Sequence[str]) -> str:
return ", ".join(CARD_LABELS[card] for card in cards)

def _default_potion(self) -> list[str]:
return DEFAULT_POTION.copy()

def _deal_damage(self, target_is_player: bool, amount: int) -> int:
if target_is_player:
lost_amount = min(amount, self.player_hp)
self.player_hp -= lost_amount
self.player_life_lost_this_round += lost_amount
return lost_amount

lost_amount = min(amount, self.ai_hp)
self.ai_hp -= lost_amount
self.ai_life_lost_this_round += lost_amount
return lost_amount

def _heal_target(self, target_is_player: bool, amount: int) -> int:
if target_is_player:
healed_amount = min(amount, MAX_HP - self.player_hp)
self.player_hp += healed_amount
return healed_amount

healed_amount = min(amount, MAX_HP - self.ai_hp)
self.ai_hp += healed_amount
return healed_amount

def _grant_random_cards(self, hand: list[str], amount: int) -> list[str]:
available_cards = list(CARD_LABELS.keys())
gained_cards: list[str] = []
for _ in range(amount):
card = self.rng.choice(available_cards)
hand.append(card)
...
def _handle_simultaneous_death(self, brewer_hand: list[str], brewer: str) -> bool:
if self.player_hp > 0 or self.ai_hp > 0:
return False

self.player_hp = min(MAX_HP, self.player_hp + self.player_life_lost_this_round)
self.ai_hp = min(MAX_HP, self.ai_hp + self.ai_life_lost_this_round)
print("Both players were knocked out in the same round.")
print(f"Player regains {self.player_life_lost_this_round} life from the round.")
print(f"AI regains {self.ai_life_lost_this_round} life from the round.")
gained_cards = self._grant_random_cards(brewer_hand, 2)
print(f"{brewer} regains 2 random cards: {self._format_card_list(gained_cards)}")
return True
...
return
if len(self.player_hand) == 0 and len(self.ai_hand) == 0:
self.game_over = True
self.result_text = "Result: Draw. Both players ran out of cards."
return
if len(self.player_hand) == 0:
self.game_over = True
self.result_text = "Result: AI wins. Player has no cards left."
return
if len(self.ai_hand) == 0:
self.game_over = True
self.result_text = "Result: Player wins. AI has no cards left."
return
self.game_over = False
...
print("After each round, the brewer gains 2 random cards.")
print("If a brewer cannot fill a potion, that potion defaults to Chaos + Chaos.")
